...
class Func {
  ...
  constructor(options) {
    ...
    this.environment = { }
    this.logger = options.logger || new ConsoleLogger({ LOG_LEVEL: this.LOG_LEVEL, prettify: options.prettify })
    this.logger.meta({ component: "funcmatic", lifecycle: "system" })
  }
  ...
  async invoke(ctx, options) {
    options = options || { } 
    this.initCtx(ctx)
    ctx.logger.meta({ component: 'funcmatic', lifecycle: 'system' })
    try {
  ...
  async invokeEnv(ctx) {
    this.initCtx(ctx)
    ctx.logger.meta({ component: 'funcmatic', lifecycle: 'env' })
  ...
  async invokeStart(ctx) {
    this.initCtx(ctx)
    ctx.logger.meta({ component: 'funcmatic', lifecycle: 'start' })
    this.started = true
    if (this.isExpired()) {
  ...
  async invokeRequest(ctx) {
    this.initCtx(ctx)
    ctx.logger.meta({ component: 'funcmatic', lifecycle: 'request' })
  ...
  async invokeError(ctx) {
    this.initCtx(ctx)
    ctx.logger.meta({ component: 'funcmatic', lifecycle: 'error' })
...
class ConsoleLogger {
  ...
  trace(...args) {
    ...
    // we use 'console.debug' because 'console.trace' automatically will print a stack trace
  ...
  fatal(...args) {
    ...
    // there is no console.fatal so we use console.error 
    this.console.error(...args)
    return args
  }
