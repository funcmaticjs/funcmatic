  ...
  async invoke(ctx, options) {
    ...
    try {
      if (this.isColdStart() || options.forceColdStart) {
        ...
        ctx.logger.debug('ENV')
        ctx.logger.debug(ctx.logger.meta())
...
class ConsoleLogger {
  ...
  prettify(line, options) {
    options = options || { }
    let d = new Date(line.time)
    let msg = `[${this.formatDate(d)}] ${line.level.toUpperCase()}: ${line.msg}`
  ...
  formatDate(d) {
    ...
    let time = `${(d.toLocaleTimeString())}.${d.getMilliseconds()}`
    return `${d.getFullYear()}-${month}-${date} ${time} ${this.formatTz(d)}`
  }
...
function afterMiddleware(ctx, fn, i) {
  ...
  ctx.logger.debug('END')
  ctx.logger.debug({ duration, execution }, 'REPORT')
}
